  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center bg-white border border-slate-200 p-4 rounded-2xl">
        <h2 className="font-heading font-extrabold text-xl text-slate-900">Package Offerings ({packages.length})</h2>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {packages.map((pkg) => (
          <div key={pkg.id} className="bg-white border border-slate-200 rounded-2xl p-6 space-y-4 flex flex-col justify-between">
            <div className="space-y-2">
              ...
              <h3 className="font-heading font-extrabold text-lg text-slate-900">{pkg.name}</h3>
              <p className="font-heading font-extrabold text-2xl text-blue-600">₹{pkg.price.toLocaleString()}</p>
              <p className="text-xs text-slate-400 line-clamp-2">{pkg.description}</p>
            </div>

            <div className="pt-4 border-t border-slate-200 text-xs text-slate-400 flex justify-between items-center">
              <span>{pkg.sessionsCount} Practical Sessions</span>
              <span className="font-mono text-[10px] text-slate-400">{pkg.id.slice(-6)}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 bg-white/10 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-3xl p-8 max-w-md w-full space-y-4 shadow-hover">
            <h3 className="font-heading font-extrabold text-xl text-slate-900">Create New Package</h3>

            <form onSubmit={handleCreatePackage} className="space-y-4 text-xs">
              <div>
                <label className="block text-slate-600 font-bold mb-1 uppercase">Package Name</label>
                <input
                  type="text"
                  required
                  value={name}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-slate-600 font-bold mb-1 uppercase">Type</label>
                  <select
                    value={type}
                ...
                <div>
                  <label className="block text-slate-600 font-bold mb-1 uppercase">Price (₹)</label>
                  <input
                    type="number"
                    required
                    value={price}
              ...
              <div>
                <label className="block text-slate-600 font-bold mb-1 uppercase">Sessions Count</label>
                <input
                  type="number"
                  required
                  value={sessionsCount}
