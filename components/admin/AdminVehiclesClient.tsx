  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-slate-900 border border-slate-800 p-4 rounded-2xl">
        <h2 className="font-heading font-extrabold text-xl text-slate-100">Learning Fleet ({vehicles.length})</h2>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {vehicles.map((veh) => (
          <div key={veh.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-3 flex flex-col justify-between">
            <div className="space-y-2">
              ...
              <h3 className="font-heading font-extrabold text-lg text-slate-100">{veh.name}</h3>
              <p className="font-mono text-xs text-slate-400">Plate: {veh.plateNumber}</p>
              <p className="text-xs text-amber-400 font-extrabold">₹{veh.ratePerSession} / session</p>
            </div>

            <div className="pt-3 border-t border-slate-800 flex justify-between items-center text-xs text-slate-400">
              <span className="text-[10px] uppercase font-bold text-emerald-400">{veh.status}</span>
              <span className="font-mono text-[10px] text-slate-500">{veh.id.slice(-6)}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 max-w-md w-full space-y-4 shadow-2xl">
            <h3 className="font-heading font-extrabold text-xl text-slate-100">Add Vehicle to Fleet</h3>

            <form onSubmit={handleCreateVehicle} className="space-y-4 text-xs">
              <div>
                <label className="block text-slate-300 font-bold mb-1 uppercase">Vehicle Model / Name</label>
                <input
                  type="text"
                  required
                  value={name}
              ...
              <div>
                <label className="block text-slate-300 font-bold mb-1 uppercase">Plate Number</label>
                <input
                  type="text"
                  required
                  value={plateNumber}
                  ...
                  className="w-full bg-slate-950 border border-slate-800 text-slate-100 p-3 rounded-xl outline-none font-mono"
                />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-slate-300 font-bold mb-1 uppercase">Tier</label>
                  <select
                    value={tier}
                ...
                <div>
                  <label className="block text-slate-300 font-bold mb-1 uppercase">Transmission</label>
                  <select
                    value={transmission}
              ...
              <div>
                <label className="block text-slate-300 font-bold mb-1 uppercase">Rate Per Session (₹)</label>
                <input
                  type="number"
                  required
