...
const server = http.createServer((req, res) => {
  ...
  if (req.url === '/') {
    ...
    res.end(`
      ...
      <html>
      <body>
        <video id="v" src="/videos/swift.mp4" crossorigin="anonymous" muted playsinline style="display:none;"></video>
        <canvas id="c"></canvas>
        <script>
...
server.listen(PORT, async () => {
  console.log(`Frame extractor listening on port ${PORT}...`);
  ...
  setTimeout(async () => {
    try {
      const jsonRes = await fetch('http://localhost:9220/json');
      ...
      ws.onmessage = (event) => {
        ...
        if (msg.id === 1) {
          if (msg.result && msg.result.result && msg.result.result.value) {
            ...
            console.log(`Received ${frames.length} frames! Writing files to disk...`);
            ...
            console.log(`✅ Successfully saved 60 frames to public/frames!`);
          } else {
            console.error('Failed evaluation result:', JSON.stringify(msg));
          }
          ws.close();
          edgeProc.kill();
          server.close();
